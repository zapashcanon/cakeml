...
open lem_pervasivesTheory semanticPrimitivesTheory astTheory libTheory bigStepTheory exhLangTheory conLangTheory decLangTheory;
...
((case (op,vs) of
    ...
    | (Op_pat (Init_global_var_i2 idx), [v]) =>
        if idx < LENGTH genv then
          ...
          NONE
    | (Op_pat (Op_i2 W8fromInt), [Litv_pat (IntLit n)]) =>
        SOME (((cnt,s),genv), Rval (Litv_pat (Word8 (i2w n))))
    | (Op_pat (Op_i2 W8toInt), [Litv_pat (Word8 w)]) =>
        SOME (((cnt,s),genv), Rval (Litv_pat (IntLit (int_of_num (w2n w)))))
