...
((case (op, vs) of
    ...
    | (W8fromInt, [Litv_i1 (IntLit n)]) =>
        SOME (s, Rval (Litv_i1 (Word8 (i2w n))))
    | (W8toInt, [Litv_i1 (Word8 w)]) =>
        SOME (s, Rval (Litv_i1 (IntLit (int_of_num (w2n w)))))
...
(store_alloc (W8array (REPLICATE (Num (ABS ( n))) w)) s)
    ...
    | (Vlength, [Vectorv_i1 vs]) =>
        SOME (s, Rval (Litv_i1 (IntLit (int_of_num (LENGTH vs)))))
