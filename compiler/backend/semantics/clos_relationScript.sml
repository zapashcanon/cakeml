...
`(!(ffi:'ffi itself) i w v v'. val_rel ffi i w v v' ⇒ ∀i'. i' ≤ i ⇒ val_rel ffi i' w v v') ∧
 ...
 >- (qpat_assum `state_rel _ P1 P2 P3` mp_tac >>
     ...
     >- (full_simp_tac(srw_ss())[LIST_REL_EL_EQN] >>
         srw_tac[][] >>
         rename1 `OPTREL (val_rel (:'ffi) N _)
...
`!c v v' vs vs' (s:'ffi closSem$state) s' loc.
 ...
 >- ((* Full, Full *)
     ...
     >- ((* LENGTH used1 < LENGTH used2 *)
         ...
         rename1 `state_rel s1.clock w s1 s2` >>
     ...
     >- ((* LENGTH used2 < LENGTH used1 *)
         ...
         full_simp_tac(srw_ss())[] >> rename1 `state_rel s2.clock w s1 s2` >>
...
`(!(ffi:'ffi itself) i w v1 v2.
 ...
 (* Closure *)
 >- (
  rename1 `val_rel _ i _ (Closure locopt argE closE n e) V` >>
