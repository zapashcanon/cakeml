...
val (v_rel_rules, v_rel_ind, v_rel_cases) = Hol_reln
  ...
  `(∀ lit lit'.
    ...
    ∀ cn cn' vs vs'.
      ...
      cn = cn' ∧ LIST_REL v_rel vs vs' ⇒ v_rel (flatSem$Conv cn vs) (flatSem$Conv cn' vs')) ∧ (
...
val alpha_eq_es_concat = Q.store_thm("alpha_eq_es_concat",
  ...
  `∀ l l' r r'.  alpha_eq_es [] l r ∧ alpha_eq_es [] l' r' ⇒ alpha_eq_es [] (l ++ l') (r ++ r')`,
...
val alpha_eq_es_reverse = Q.store_thm("alpha_eq_es_reverse",

  `∀ es l. alpha_eq_es [] es l ⇒ alpha_eq_es [] (REVERSE es) (REVERSE l)`,

   Induct
>> Cases_on `l`
>> rw[alpha_eq_exp_def]
>> `alpha_eq_es [] [h'] [h]` by rw[alpha_eq_exp_def]
>> metis_tac[alpha_eq_es_concat])

val alpha_eq_es_LENGTH = Q.store_thm("alpha_eq_es_LENGTH",

  `∀ es l. alpha_eq_es [] es l ⇒  LENGTH es = LENGTH l`,
